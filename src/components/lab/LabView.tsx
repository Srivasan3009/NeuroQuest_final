...
import { KidsLabPlayground } from "./KidsLabPlayground";
...
export const LabView: React.FC<LabViewProps> = ({
  ...
  const [activeLabSection, setActiveLabSection] = useState<"kids" | "goals">("kids");

  const handleEarnKidsReward = (sparks: number, xp: number) => {
    onAddSparks(sparks);
  };

  return (
    <div id="lab-view" className="w-full max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 pt-3 sm:pt-6 pb-32 space-y-6">
      {/* 1. Header with Sparks pill & Section Navigation */}
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 pb-1">
        ...
        <div className="flex items-center gap-2">
          <div
            className={`p-1 rounded-2xl flex items-center gap-1 ${
              isNeumorphic
                ? "neu-inset"
                : isRiso
                ? "bg-[#FFFDF9] border-2 border-[#1E1B18]"
                : "bg-slate-900 border border-slate-800"
            }`}
          >
            <button
              id="btn-lab-kids-section"
              onClick={() => {
                soundFx.playTap();
                setActiveLabSection("kids");
              }}
              className={`px-3.5 py-1.5 rounded-xl text-xs font-bold uppercase tracking-wider flex items-center gap-1.5 transition-all ${
                activeLabSection === "kids"
                  ? isNeumorphic
                    ? "neu-flat text-indigo-600 font-black"
                    : isRiso
            ...
            <button
              id="btn-lab-goals-section"
              onClick={() => {
                soundFx.playTap();
                setActiveLabSection("goals");
              }}
              className={`px-3.5 py-1.5 rounded-xl text-xs font-bold uppercase tracking-wider flex items-center gap-1.5 transition-all ${
                activeLabSection === "goals"
                  ? isNeumorphic
                    ? "neu-flat text-amber-600 font-black"
                    : isRiso
                    ? "bg-[#FEF08A] text-[#1E1B18] border border-[#1E1B18]"
          ...
          <div
            className={`flex items-center gap-1.5 px-3.5 py-2 rounded-2xl text-xs font-mono font-bold ${
              isNeumorphic
      ...
      {activeLabSection === "kids" ? (
        <KidsLabPlayground onEarnReward={handleEarnKidsReward} theme={theme} />
      ) : (
        <div className="space-y-6">
          {/* League Chest Progress Card */}
          <div
